...
void enleveCoccinelle(int coccinelleIndex, ensembleCoccinelle *ensCoc, int n, int p, Case potager[n][p])
{
    ...
    (*ensCoc).tab[coccinelleIndex] = (*ensCoc).tab[(*ensCoc).nbCoccinelle - 1];
    (*ensCoc).tab[coccinelleIndex].index = coccinelleIndex;
    (*ensCoc).nbCoccinelle = (*ensCoc).nbCoccinelle - 1;
    potager[(*ensCoc).tab[coccinelleIndex].coordCoccinelle.x][(*ensCoc).tab[coccinelleIndex].coordCoccinelle.y].coccinelleCase = NULL;
}
